...
export function TransactionContactsScreen({navigation}: NativeStackScreenProps<RootStackParamList>) {
    ...
    const listItem = (c: any) => {
        return <List.Item title={c.name} description={c.address} key={c.address}
                          right={props =>
                              <Button onPress={() => onSelectContact(c.address)}
                                      style={{alignSelf: "center"}}>Select</Button>
                          }/>
    ...
    return (
        <View style={{flex: 1}}>
            ...
            <SafeAreaView style={{flex: 1}}>
                ...
                <Button style={{width: 150, alignSelf: 'flex-end', margin: 10}}
                        compact={true}
                        mode={'contained'}
